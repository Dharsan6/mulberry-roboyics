...
from backend.app.schemas.telemetry import AgronomicAnalysisResponse, PlantationSummaryResponse
...
def get_plantation_summary(db: Session = Depends(get_db)):
    ...
    if total_samples > 0:
        avg_ph = db.query(func.avg(SoilObservationModel.ph)).scalar() or 0.0
        avg_ec = db.query(func.avg(SoilObservationModel.ec)).scalar() or 0.0
        avg_moisture = db.query(func.avg(SoilObservationModel.moisture)).scalar() or 0.0
        ...
        avg_ph, avg_ec, avg_moisture = 6.8, 0.75, 45.0
